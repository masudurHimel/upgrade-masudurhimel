...
## Steps
1. **Sync down first** — if this is a git repo, run `git pull` to fetch the latest `PROGRESS.md`/`ROADMAP.md` before anything else (prevents desktop↔mobile drift). Skip only if offline or not a repo. Then load coaching context. On desktop this comes from auto-memory (`learning-program.md`, `user-profile.md`, `reply-style.md`, `teaching-style.md`, `card-teaching-style.md`, `explicit-topic-teach.md`, `review-rules.md`). **On mobile/cloud there is no auto-memory — read the in-repo mirror `coaching-memory/*.md` instead** (same files). Then read `PROGRESS.md` for current state + dates.
...
6. **Sync up automatically** — after writing `PROGRESS.md`, run `git add -A && git commit -m "teach: <topic> — <YYYY-MM-DD>" && git push` on your own (no manual step, Masudur-only author, no Claude co-author). If push is rejected, `git pull` then push again. See root `CLAUDE.md` §Auto-sync.
