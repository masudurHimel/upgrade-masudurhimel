...
> **Created:** 2026-08-22 · **Checkpoint:** ~2026-10-24 · **Last updated:** 2026-09-23 (learning day 17)
...
Artifacts (written)  0/3        Tests passed   0        Teach-back ✓   2 topics (12 cards)
```

**Next up:** 🔁 **Day-18 REVIEW** (BE1.3 cards 1–2: handshake cost vs app time · the two kernel queues, full-vs-deep accept queue ⇒ night B vs night C). After it: **BE1.3 card 3/5 — sequence numbers & what's negotiated**, from the 2026-09-22 bucket.
...
|------|-------|-----------|--------|
| **2026-09-22** | BE1.3 TCP 3-way handshake | ~~(2) kernel queues — SYN queue / accept queue — cleared 2026-09-23~~ · (3) sequence numbers & what's negotiated · (4) failure modes — SYN retries, RST vs silence, half-open · (5) edge cases + staff lens — SYN flood, handshake cost at scale, TFO | **🟡 open** (3 cards left) |
...
| ~~2026-09-02~~ | ~~BE1.2 DNS resolution~~ | ~~(2) caching at every layer — cleared 2026-09-05 · (3) TTL mechanics & trade-offs — cleared 2026-09-06 · (4) record types + failure modes — cleared 2026-09-19 · (5) edge cases + staff lens (negative caching, K8s `ndots`) — cleared 2026-09-20~~ | **✅ closed 2026-09-20** — BE1.2 `[✓]` |
...
| ~~2026-09-21~~ | ~~Day-15 review miss (Q2)~~ | ~~stale cache ⇒ old-IP success, never NXDOMAIN — a cache is a photocopy of a real answer; every pre-deletion copy is positive, so a stale copy can only replay success~~ | **✅ closed 2026-09-21** (same-session teach-back: C's old-IP success = warm stale cache, D's NXDOMAIN = fresh query ⇒ deletion real; pre-deletion caches hold only positives) |
...
- [~] BE1.3 TCP connection setup — 3-way handshake *(cards 1–2/5 ✓ 2026-09-22 → 2026-09-23 — SYN/SYN-ACK/ACK, why three, connect() = 1 RTT · SYN queue / accept queue, `accept()`, full-queue SYN drop; cards 3–5 in bucket)*
...
|------|----------------|-------|-------|
| 2026-09-23 | BE1.3 card 2/5 — kernel queues: **SYN queue** (half-done handshakes, waiting on the client's final ACK) → **accept queue / backlog** (finished connections the app hasn't `accept()`ed yet); `connect()` returns the moment a connection lands in the accept queue — the worker is never involved | 1 | Learning day 17. 2-min box → 1 card from the 2026-09-22 bucket; cards 3–5 still parked. Self-check **2/2**: ✓ Q1 connect OK ⇒ established + sitting in backlog, view not running, proves the accept queue had room; ✓ Q2 capped queue ⇒ handshakes dropped ⇒ connect timeout with server up (night B). **Teach-back passed** — both queues, `accept()` as the exit of queue 2, and "connect() is kernel↔kernel, the worker doesn't gossip with it" unprompted. Polished ×2: (a) queue 1 → queue 2 is triggered by the *client's final ACK arriving*, not the kernel working through the line; (b) on a full accept queue Linux drops the **SYN itself — no SYN-ACK is sent** (he said kernel SYN-ACKs then drops) ⇒ client retransmits SYN 1s/2s/4s ⇒ connect timeout = night B; rarer final-ACK-drop variant ⇒ client thinks connected ⇒ night C. Contrast shipped: busy workers + queue *deep* ⇒ fast connect then silence (C) · queue *full* ⇒ connect hangs (B). Staff lens: `ss -ltn` Recv-Q/Send-Q, `nstat` ListenOverflows/ListenDrops before blaming the network. Links back: day-6 triad (B vs C now mapped to queue state), BE1.3 card 1 (kernel answers the handshake). Forward: BE3.1 `listen(backlog)` / `somaxconn`, BE3.7 backpressure, BE5.4 shared listening socket across pre-fork workers. Next: **day-18 REVIEW**. |
...
| 2026-09-13 | 🔁 **Day-12 REVIEW** — 2 applied questions (BE1.2 cards 2–3 + flagged resurface) | — | Learning day 12. 2-min box → 2 questions. **1 clean / 1 half.** ✓ Q2 supplier drops TTL to 5s: every cache layer goes cold every 5s ⇒ frequent full lookups, latency + resolver load at every layer — sharpened with the ~720× query-load figure vs TTL 3600 and that connection pools dampen it (a lookup only fires on a *new* connection). ~ Q1 self-healing pods: nailed dial 1 (pool churn ⇒ new connection ⇒ a lookup runs) but skipped dial 2 — **the flagged day-10 miss, again**: why the lookup returned the NEW IP. Re-taught with the fridge rule (going to the fridge ≠ fresh milk; the carton must be past expiry): cache alive ⇒ new connection still gets the old IP; **pool churn pulls the trigger, TTL expiry loads fresh ammo** → bucketed. Teach-back (TTL 24h variant) **passed**: pods recover only when *their own* cached copy expires — per-cache clocks ⇒ some flip, most don't inside the outage window → **cleared same session**. Polished: restart fixes a *held* IP/connection, not a genuinely cached record (CoreDNS/VPC still serve it until expiry/flush). ⚠️ 2nd miss on this item ⇒ resurface once more on day-15 review. Next session: BE1.2 card 4 (bucket 2026-09-02). |
| 2026-09-06 | BE1.2 card 3/5 — TTL mechanics & trade-offs: owner (authoritative) stamps the TTL, each cache counts down from *its own* store time; the dial is agility vs lookup load (milk-carton expiry) | 1 | Learning day 11. 2-min box → 1 card from the 2026-09-02 bucket; cards 4–5 still parked. Teach-back (supplier's "TTL 30, no downside") in two halves: ✓ **"when 30s still won't save you"** nailed unprompted — pods with the old IP in config / pooled connections never re-look-up, TTL only changes a lookup that *happens* (card-2 rule held). ~ First half (the downside) skipped on pass 1 → hinted via milk carton → then ✓ "lookup every 30s". Sharpened: cache goes cold every 30s ⇒ real requests pay the resolver round trip + ~120× DNS query load vs TTL 3600 — *their fast failover is your permanent small tax*. **Passed.** Edge cases shipped: resolver TTL clamping, TTL 0. Links back: BE1.1 card 2 + migration play, BE1.2 card 2. Next session = **day-12 review** (must resurface: TTL expiry as self-recovery, flagged day 10). |
| 2026-09-05 | BE1.2 card 2/5 — caching at every layer: a *stack* of independent caches (app process → stub → CoreDNS → VPC resolver), each copy on its own TTL clock | 1 | Learning day 10 (second session of the date; day-9 review was earlier). 2-min box → 1 card from the 2026-09-02 bucket; cards 3–5 still parked. Teach-back scenario (stale pod vs fresh pod on same CoreDNS): first said "the stub" → corrected — the stub **forwards only, holds nothing**; the stale copy lives in the **app process** (IP saved at startup, or a pooled connection to the old IP). TTL-lowering probe answered as generic cache expiry → corrective contrast table: a held IP/connection is **not a cache entry — nothing is counting down**; *TTL can only speed up a lookup that is going to happen, it can't cause one that never happens*; restart is fix + proof. Final split-groups teach-back: broken-pod half **✓ clean** (kept config, no TTL relation, never re-looks-up); self-heal half missed twice ("restart", then "new connection pools") → re-taught the two dials: **pool churn decides *when* a lookup runs; TTL expiry decides *what answer* it gets**. Core **passed**; ⚠️ resurface on day-12 review: *TTL expiry as the self-recovery mechanism*. Builds on BE1.1 card 2 (client-side IP caching vs TTL — same miss family). |
| 2026-09-05 | 🔁 **Day-9 REVIEW** — 2 applied questions (BE1.1 + BE1.2 card 1) | — | Learning day 9. 2-min box → 2 questions. **Q1 ✓** payment-provider DNS migration: authoritative box + who runs it (provider, e.g. Cloudflare — the day-8 "who runs it" half-miss is **cleared**), TTL play correct. Polished: zero errors because the **old IP keeps answering** during the overlap; how long a pod stays stale = the TTL stamped when *its* cache stored the record. **Q2 ✗** the two-404s contrast (from day 7) slipped: answered A only and blamed nginx — re-taught that in A nginx routed fine and **Django's URL resolver** 404'd (wrong path, your log has the request); B skipped — wrong **`Host`** → no `server_name` match → **default server** 404s, your app logs nothing. Tell restated: *404 in your app's log = wrong path; 404 in the default server's log = wrong Host.* → bucketed, then **cleared same session**: teach-back nailed the fork (request in your app's log ⇒ wrong path; absent ⇒ Host/`server_name` mismatch, default server answered). |
