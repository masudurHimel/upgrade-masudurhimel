...
**Next up:** ⚠️ **Carry-over from 2026-08-16 (review bucket, 1 item: `__slots__` mechanism)** — teach it back without notes before any new cards. Then M1.11 slice 2 (more concurrency patterns).
...
> **OPEN — 2026-08-16 (Day-24 review, deferred): 1 item.** Q3 `__slots__` (M1.2) missed — answered "lose flexibility / removes exponential memory leak"; missed that it removes the **per-instance `__dict__`**, and mis-framed a per-object size cost as a leak. Second drift on this topic (also shaky in the 2026-07-06 day-10 review). Re-taught same session (per-instance dict ≈100+ bytes × N; slots = struct-like fixed array; lose runtime attribute-add / `__weakref__` / dict-less subclassing). **Must teach back without notes before new material.**
...
| 2026-08-16 | Day-24 review (deferred) — rapid recall M1.11/M1.10 + M1.2, 2-min | — | 2/3. Q1 Semaphore ✓ — #21 queues, named 429; sharpened the *bigger* failure = self-inflicted resource exhaustion (sockets/pool/memory) before the vendor ever throttles — valve protects you first. Q2 cancellation ✓ — "loop finds a state where it can throw cancel error"; sharpened the missing half: because `CancelledError` is *thrown in* at the pause point it unwinds like a normal exception, so `finally`/`__exit__` fire (a killed task would leak). Q3 `__slots__` ✗ → re-taught + bucketed: removes the per-instance `__dict__` (hash table, ~100+ bytes/obj) not a "leak"; slots = fixed struct-like array; cost = no runtime attribute-add, no `__weakref__`, dict returns in a slot-less subclass. Staff lens: hot-path tool (millions of objects), not a style rule. Ties M1.8 (stop paying per-item overhead at scale). |
