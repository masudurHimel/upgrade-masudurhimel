...
```
PATHS.md              ← the registry (name resolver + shortcuts), at repo ROOT
paths/
...
`PATHS.md` resolves names and caches a summary row per path. **A path's own `PROGRESS.md` is canonical for its state** — if the registry and a `PROGRESS.md` disagree, `PROGRESS.md` wins and the registry row gets fixed.
...
| `/teach-<slug> [duration]` | that path, next in sequence — **every path gets its own command** (generated by `/create_path`; path pinned, no resolution) |
