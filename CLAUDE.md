...
2. Then read `PATHS.md` (the path registry) to resolve which path this session belongs to, and that path's `paths/<slug>/PROGRESS.md` (current state, dates, carry-over buckets) + `paths/<slug>/ROADMAP.md` (the plan). No path named ⇒ `default`.
...
- **`PATHS.md` is the registry** (name resolver + cached summary row per path). A path's own `PROGRESS.md` is **canonical** for its state; if they disagree, fix the registry.
...
- **Every path gets its own `/teach-<slug>` command**, generated by `/create_path` (e.g. `/teach-be`). It pins the path and takes duration/topic args only. All other skills take the shortcut as an argument instead: `/progress be`, `/test_me be`, `/assess be`.
...
2. **Immediately after writing any coaching state file** (`paths/<slug>/PROGRESS.md`, `paths/<slug>/ROADMAP.md`, `PATHS.md`, `coaching-memory/*`), automatically run:
